...
func postServerBackup(c *gin.Context) {
	...
	case backup.BorgBackupAdapter: // <-- NUEVO: soporte Borg
		adapter = backup.NewBorg(client, data.Uuid, data.Ignore)
	default:
...
func postServerRestoreBackup(c *gin.Context) {
	...
	var data struct {
		// <-- NUEVO: agrega "borg" al binding para permitir ese adapter
	...
	if data.Adapter == backup.BorgBackupAdapter {
		b := backup.NewBorg(client, c.Param("backup"), "")
...
func deleteServerBackup(c *gin.Context) {
	// <-- NUEVO: permite borrar Borg con ?adapter=borg
	if c.Query("adapter") == string(backup.BorgBackupAdapter) {
		b := backup.NewBorg(middleware.ExtractApiClient(c), c.Param("backup"), "")
	...
	// Comportamiento existente (local)
